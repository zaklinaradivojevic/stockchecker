  ...
  app.route('/api/stock-prices').get(async function (req, res) {
    ...
    try {
      ...
      for (let i = 0; i < symbols.length; i++) {
        ...
        if (!response.ok) {
          console.error(`Failed to fetch stock price for ${symbol}. Response status: ${response.status}`);
        ...
        if (like === 'true' || like === true) {
          ...
          const existingLike = await Stock.findOne({ ipAddress, symbol });

          if (existingLike) {
            likes = Number(existingLike.likes);
          } else {
            const newLike = new Stock({ ipAddress, symbol, likes: 1 });
            await newLike.save();
            likes = newLike.likes;
          }
        }
